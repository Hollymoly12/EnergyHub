# F4 — RFQ List Page Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Créer la page publique `/rfq` qui liste les appels d'offres avec sidebar filtres, accès limité à 12 cartes pour les non-connectés.

**Architecture:** Server Component `page.tsx` pour le SSR + détection auth, Client Component `RFQClient.tsx` pour les filtres interactifs et la grille. Les filtres q/type sont envoyés à l'API ; les filtres budget/deadline/tags/région/actorType sont appliqués côté client sur les données fetchées.

**Tech Stack:** Next.js 15 App Router, Tailwind CSS vanilla, classes custom globals.css (`.card`, `.btn-primary`, `.btn-secondary`, `.input`, `.label`). Couleurs : `bg-slate-900`, `border-slate-800`, `text-yellow-500`, `text-slate-400`.

---

### Task 1 : Server Component `page.tsx`
...
**Step 3: Commit**
```bash
git add src/app/rfq/page.tsx
git commit -m "feat: F4 rfq page server component"
```

---

### Task 2 : Client Component `RFQClient.tsx`

**Files:**
- Create: `src/app/rfq/RFQClient.tsx`
...
export default function RFQClient({ initialRFQs, totalCount, isLoggedIn }: Props) {
  ...
  const hasClientFilters = actorTypeFilter || regionFilter || budgetFilter || deadlineBefore || tagFilter;
  const hasAnyFilter = search || typeFilter || hasClientFilters;
...
**Step 3: Commit**
```bash
git add src/app/rfq/RFQClient.tsx
git commit -m "feat: F4 rfq list client component + filters"
```

---

### Task 3 : Push & vérification
...
- `/rfq` affiche la sidebar + grille de cartes
- Non-connecté : 12 cartes + CTA bloc
- Connecté : liste complète + bouton "Publier un RFQ"

**Step 3: Vérifier les filtres**
- Recherche texte → appel API avec `?q=...`
- Type RFQ/RFI → appel API avec `?type=...`
- Filtre acteur ciblé → filtrage client immédiat
