...
export default async function ActorProfilePage({
  ...
  const typeInfo = ACTOR_LABELS[org.actor_type] || { label: org.actor_type, icon: "business" };
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <Link href="/directory" className="text-sm text-slate-500 hover:text-primary transition-colors inline-flex items-center gap-1.5 mb-6">
          ← Retour à l'annuaire
        </Link>

        {/* Cover */}
        <div className="bg-white rounded-3xl border border-black/5 overflow-hidden mb-6">
          <div
            className="h-32 bg-gradient-to-r from-slate-100 to-slate-200"
          ...
          <div className="px-6 pb-6">
            <div className="flex items-end gap-4 -mt-8 mb-4">
              <div className="w-16 h-16 rounded-xl bg-slate-100 border-2 border-white flex items-center justify-center overflow-hidden shrink-0">
                {org.logo_url
                  ? <img src={org.logo_url} alt="" className="w-full h-full object-contain" />
                  : <span className="material-symbols-outlined text-3xl text-slate-500">{typeInfo.icon}</span>}
              </div>
              <div className="pb-1">
                ...
                <div className="text-sm text-slate-500 mt-0.5 flex items-center gap-1">
                  <span className="material-symbols-outlined text-base leading-none">{typeInfo.icon}</span> {typeInfo.label}
                  {org.city && ` · ${org.city}`}
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {org.description && (
              <div className="bg-white rounded-3xl p-6 border border-black/5">
                <h2 className="text-sm font-semibold text-primary mb-3">À propos</h2>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {org.technologies.map((tech: string) => (
                        <span key={tech} className="text-xs px-3 py-1 rounded-full bg-slate-100 text-slate-600 border border-slate-200">
                          {tech}
                        </span>
                      ))}
                    </div>
            ...
            {(org.team_size || org.annual_revenue) && (
              <div className="bg-white rounded-3xl p-6 border border-black/5">
                <h2 className="text-sm font-semibold text-primary mb-3">Informations</h2>
                <div className="grid grid-cols-2 gap-4">
                  {org.team_size && (
                    <div>
                      <div className="text-xs text-slate-500 mb-0.5">Équipe</div>
                      <div className="text-sm text-primary">{org.team_size} employés</div>
                    </div>
                  )}
                  {org.annual_revenue && (
                    <div>
                      <div className="text-xs text-slate-500 mb-0.5">Chiffre d'affaires</div>
                      <div className="text-sm text-primary">{org.annual_revenue}</div>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="bg-white rounded-3xl p-5 border border-black/5">
              ...
              {isLoggedIn ? (
                <div className="space-y-3">
                  ...
                  >
                    <span className="material-symbols-outlined text-sm leading-none align-middle">mail</span> Contacter
                  </button>
                </div>
              ) : (
                <div className="relative">
                  <div className="space-y-3 select-none blur-sm pointer-events-none">
                    <div className="flex items-center gap-2.5 text-sm text-slate-500">
                      <span className="material-symbols-outlined text-base leading-none">language</span><span>www.example.be</span>
                    </div>
                    <div className="flex items-center gap-2.5 text-sm text-slate-500">
                      <span className="material-symbols-outlined text-base leading-none">call</span><span>+32 2 000 00 00</span>
                    </div>
                    <div className="flex items-center gap-2.5 text-sm text-slate-500">
                      <span className="material-symbols-outlined text-base leading-none">work</span><span>LinkedIn</span>
                    </div>
                  </div>
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/90 rounded-2xl text-center p-3">
                    <div className="text-sm font-semibold text-primary mb-1 flex items-center gap-1 justify-center">
                      <span className="material-symbols-outlined text-base leading-none">lock</span> Coordonnées masquées
                    </div>
