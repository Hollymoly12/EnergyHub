...
CREATE POLICY "rfq_responses_read" ON rfq_responses FOR SELECT USING (
  organization_id IN (SELECT organization_id FROM members WHERE id = auth.uid())
  OR rfq_id IN (
    SELECT id FROM rfqs WHERE organization_id IN (SELECT organization_id FROM members WHERE id = auth.uid())
  )
);
...
CREATE POLICY "deal_interests_read" ON deal_interests FOR SELECT USING (
  ...
  OR deal_id IN (
    SELECT id FROM deals WHERE organization_id IN (SELECT organization_id FROM members WHERE id = auth.uid())
  )
);
